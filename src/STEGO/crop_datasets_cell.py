...
class RandomCropComputer(Dataset):
    ...
    def __init__(self, cfg, dataset_name, img_set, crop_type, crop_ratio):
        ...
        self.save_dir = join(
            cfg.pytorch_save_data_dir, "cropped", "{}_{}_crop_{}".format(dataset_name, crop_type, crop_ratio))
    ...
    def __getitem__(self, item):
        ...
        for crop_num, (img, label) in enumerate(zip(imgs, labels)):
            ...
            # img_arr = img.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu').numpy().astype(np.uint16)
